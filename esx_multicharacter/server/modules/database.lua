...
function Database:DeleteCharacter(source, charid)
    ...
    local identifier = ("%s%s:%s"):format(Server.prefix, charidStr, Server:GetIdentifier(source))
    local query = "DELETE FROM `%s` WHERE %s = ?"
    local queries = {}
    local count = 0

    -- First disable the slot
    self:DisableSlot(Server:GetIdentifier(source), charid)

    -- Then delete the character data
    ...
    MySQL.transaction(queries, function(result)
        if result then
            ...
            -- Remove the slot after successful deletion
            self:RemoveSlots(Server:GetIdentifier(source))
